...
#pragma omp critical
				...
				if (m_sample > 0) {
					...
					dtype ran = (sqrt(target_vob.m_id_to_freq[ctxi] / (m_sample * target_vob.m_allword_count)) + 1) * (m_sample * target_vob.m_allword_count) / target_vob.m_id_to_freq[ctxi];
					next_random = next_random * (unsigned long long)25214903917 + 11;
					if (ran < (next_random & 0xFFFF) / (dtype)65536) continue;
				}
				...
				NRVec<dtype> cur_source_emb(dim);
				neu1e = 0;
				cur_source_emb = 0;
				int source_size = wrdi.size();
				for (int s = 0; s < source_size; s++) {
					for (int c = 0; c < dim; c++) {
						cur_source_emb[c] += source_embs[wrdi[s]][c];
					}
				}
